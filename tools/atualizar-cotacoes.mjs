 ...
 *
 *   node tools/atualizar-cotacoes.mjs carteira.json [--token SEU_TOKEN]
 *
...
async function main() {
  const { arquivo, token } = lerArgumentos(process.argv);
  if (!arquivo) {
    console.error('Uso: node tools/atualizar-cotacoes.mjs <carteira.json> [--token SEU_TOKEN]');
    process.exit(2);
  }
  ...
  const { dados, erros } = await buscarCotacoes(tickers, { token });
